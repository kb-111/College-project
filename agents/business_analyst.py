...
class BusinessAnalystAgent:
    ...
    def generate_user_stories(self, business_requirements):
        ...
        # Generate user stories using Ollama model
        try:
            # Use Ollama's parameters
            generated_output = self.llm.invoke(
                formatted_prompt,
                temperature=0.7,
                top_p=0.95,
                max_tokens=1024
            ...
            # Simplified fallback
            generated_output = self.llm.invoke(formatted_prompt)
        
        # Handle the output - Ollama should return a string directly
        result = str(generated_output)
