...
class TesterAgent:
    def __init__(self, model=None, db_manager=None):
        """
        Initialize the TesterAgent with an Ollama model
        ...
        if model is None:
            ...
            self.pipeline = Ollama(model="llama2", base_url="http://localhost:11434")
        elif isinstance(model, str):
    ...
    def generate_test_cases(self, user_stories, code):
        ...
        try:
            ...
            print(f"Error generating test cases with Ollama: {e}")
            # Simple fallback
            result = self.pipeline.invoke(prompt)
            ...
            if self.db_manager:
                self.db_manager.store_artifact(
                    ...
                    metadata={
                        ...
                        "language": "python",
                        "framework": "pytest",
                        "user_story_ids": user_story_id_string
                    }
                )
    ...
    def execute_tests(self, code, test_cases):
        ...
        try:
            ...
            print(f"Error executing tests with Ollama: {e}")
            result = self.pipeline.invoke(prompt)
