...
# class AgentManager:
#     def __init__(self):
#         self.llm = Ollama(model="codellama:7b")
...
#         self.pm_agent = ProjectManagerAgent(self.llm, self.db_manager)
...
class AgentManager:
    ...
    def process_business_requirements(_self, requirements):
        """Process initial business requirements and generate user stories"""
        return _self.ba_agent.generate_user_stories(requirements)
    
    @st.cache_data(show_spinner=False)
    def develop_code(_self, user_stories):
        """Develop code based on user stories"""
        return _self.dev_agent.generate_code(user_stories)
    
    @st.cache_data(show_spinner=False)
    def create_test_cases(_self, user_stories, code):
        """Create test cases based on user stories and code"""
        return _self.test_agent.generate_test_cases(user_stories, code)
    
    @st.cache_data(show_spinner=False)
    def execute_tests(_self, code, test_cases):
        """Execute tests against the code"""
        return _self.test_agent.execute_tests(code, test_cases)
    
    @st.cache_data(show_spinner=False)
    def get_project_status(_self, query):
        """Get project status based on PM query"""
        return _self.pm_agent.get_status(query)
